...
- Concurrent job guard — blocks a second sort/find if a job is already running
- Destination path validation with opt-in creation (create_destination=True)
- Enrolled-face count fix — reads enrolled_faces key correctly from backend payload
- Daemon lifecycle overhaul — no more backend clones, SIGTERM stop, PID-file-based control
- Preset path resolution — resolve_path_preset maps preset names to real paths
- File-count cross-check — flags total_files vs scanned-file mismatches in status
- Trimmed MCP system instructions to survive client truncation; safety rules now first
- Six new test files covering all new helpers (153 tests total)
...
| Platform | File | Type |
|----------|------|------|
| macOS (Apple Silicon) | [locallens-agent-macos-arm64.dmg](https://github.com/ashesbloom/locallens_mcp_agent/releases/download/v1.0.31/locallens-agent-v1.0.31-macos-arm64.dmg) | Menu Bar App |
| macOS (Apple Silicon) | [locallens-mcp-macos-arm64.zip](https://github.com/ashesbloom/locallens_mcp_agent/releases/download/v1.0.31/locallens-mcp-v1.0.31-macos-arm64.zip) | MCP Binary |
| Windows (x64) | [locallens-mcp-windows-x86_64.zip](https://github.com/ashesbloom/locallens_mcp_agent/releases/download/v1.0.31/locallens-mcp-v1.0.31-windows-x86_64.zip) | MCP Binary |
| Linux (x64) | [locallens-mcp-linux-x86_64.tar.gz](https://github.com/ashesbloom/locallens_mcp_agent/releases/download/v1.0.31/locallens-mcp-v1.0.31-linux-x86_64.tar.gz) | MCP Binary |
...
1. Download `locallens-agent-v1.0.31-macos-arm64.dmg`
...
\`\`\`bash
# Extract and set up
tar -xzf locallens-mcp-v1.0.31-macos-arm64.tar.gz   # or .zip on macOS
