...
| `README.md` | The free-preview callout under the tagline, the one-line note directly under the `## Free vs Pro` heading itself ("The tiers below are how tools are grouped long-term…"), the "Pro Tools" subheading note, and the "Activating Pro" section are all **hand-written, not flag-driven** (static markdown can't read `FREE_PREVIEW`). Remove all four, restore "Require an active Pro license" and "Purchase a license at…" — the Free/Pro tables and badges are unaffected and stay |
| `release_notes/release_notes_template.md` | The paid-mode "🔑 Free vs Pro" section is preserved as an HTML comment directly above the current "🎉 Free preview" section — swap which one is live. Also **hand-written, not flag-driven**, since it's a static template read by `scripts/set_version.py`. **Note:** until this swap happens, `set_version.py` copies that commented-out block verbatim into every new `release_notes_v<X>.md` it generates (its `{RELEASE_SECTIONS}` substitution has no comment-awareness) — invisible on the rendered GitHub page, but strip it from each dated file by hand before committing, same as was done for `v1.1.0` |
